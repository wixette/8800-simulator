...
They are listed in the order the simulator's menu offers them: first
the programs that speak through the front panel, then the ones that
speak through the teletype, each group from the simplest up.

| Program | Size | Watch | What it does |
| --- | --- | --- | --- |
| [pattern-shift.asm](pattern-shift.asm) | 8 B | Panel | Rotates 8CH across the data LEDs |
| [io-echo.asm](io-echo.asm) | 7 B | Panel | Echoes the sense switches to the data LEDs |
| [adder.asm](adder.asm) | 14 B | Panel | Adds the bytes at 0080H and 0081H, sum at 0082H |
| [bouncing-light.asm](bouncing-light.asm) | 59 B | Panel | A dark bit bounces across the data LEDs, speed set by the switches |
| [kill-the-bit.asm](kill-the-bit.asm) | 24 B | Panel | The 1975 game by Dean McDaniel ([how to play](kill-the-bit.md)) |
| [tty-hello.asm](tty-hello.asm) | 39 B | Teletype | Prints `HELLO, WORLD!` and halts |
| [tty-ascii.asm](tty-ascii.asm) | 20 B | Teletype | Prints every printable ASCII character, then halts |
| [tty-echo.asm](tty-echo.asm) | 16 B | Teletype | Echoes what you type |
...
The simulated machine starts with 256 bytes of memory, as the Altair
8800 did in its base configuration, and every program here fits in it -
below 0100H, data and stack included - so none of them needs a memory
board installed. It also means the address LEDs above A7 stay dark for
...
Write a listing next to the others and the tests pick it up. The
format is:
...
`device` is `panel` or `teletype`: whichever the program speaks
through, so the simulator can say where to watch it. Then address,
bytes, source, and an optional `LABEL:` before the source. A `DB`
line carries data rather than code. The bytes in the listing are the
program; the hex block above is the only other copy, and the tests
check it matches.

To offer it in the simulator's menu, add its name to `panel.EXAMPLES`
in [../js/panel.js](../js/panel.js), in its place in the order.
...
addresses run straight through memory, that the program stays inside
its own code when it runs, that it fits in 256 bytes, and that the
menu offers it. Add a test to
