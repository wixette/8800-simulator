...
l10n.MESSAGES = {
    ...
    'tty-hint': {
        'en': 'Type here. Nothing echoes by itself - the machine has to be running a program that reads the serial board, so pick tty-echo from Example Programs in the Debugger tab and RUN it first.',
        'es': 'Escribe aquí. Nada se repite por sí solo: la máquina tiene que estar ejecutando un programa que lea la placa serie, así que elige tty-echo en Programas de Ejemplo, en la pestaña Depurador, y ejecútalo primero.',
        'fr': 'Tapez ici. Rien ne s\'affiche tout seul : la machine doit exécuter un programme qui lit la carte série, alors choisissez tty-echo dans Programmes d’Exemple, dans l\'onglet Débogueur, et lancez-le d\'abord.',
        'de': 'Hier tippen. Nichts erscheint von selbst - die Maschine muss ein Programm ausführen, das die serielle Karte liest. Wählen Sie also zuerst tty-echo unter Beispielprogramme im Debugger-Tab und starten Sie es.',
        'it': 'Scrivi qui. Nulla viene ripetuto da solo: la macchina deve eseguire un programma che legge la scheda seriale, quindi scegli tty-echo da Programmi di Esempio nella scheda Debugger ed eseguilo prima.',
        'zh': '在这里输入。字符不会自动回显——机器必须正在运行一个读取串口板的程序，所以请先在调试器标签页的“示例程序”中选择 tty-echo 并运行它。',
        'zh-TW': '在這裡輸入。字元不會自動回顯——機器必須正在執行一個讀取串列埠板的程式，所以請先在除錯器分頁的「範例程式」中選擇 tty-echo 並執行它。',
        'ja': 'ここで入力します。文字は自動では表示されません。シリアルボードを読むプログラムを動かしている必要があるので、まずデバッガータブの「サンプルプログラム」から tty-echo を選んで RUN してください。',
        'ko': '여기에 입력하세요. 문자는 저절로 표시되지 않습니다. 직렬 보드를 읽는 프로그램이 실행 중이어야 하므로, 먼저 디버거 탭의 "예제 프로그램"에서 tty-echo를 골라 실행하세요.',
    },
    ...
    'basic-2': {
        ...
        'es': 'Pulsa Cargar 4K BASIC. La máquina se enciende, la cinta se lee por ti y se pulsa RESET.',
        'fr': 'Cliquez sur Charger 4K BASIC. La machine s\'allume, la bande est lue pour vous et RESET est appuyé.',
        'de': 'Klicken Sie auf 4K BASIC laden. Die Maschine geht an, das Band wird für Sie eingelesen und RESET gedrückt.',
        'it': 'Premi Carica 4K BASIC. La macchina si accende, il nastro viene letto per te e viene premuto RESET.',
